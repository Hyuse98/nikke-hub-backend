...
class IncreaseNikkeAttractionCaseTest {

    private val nikkeRepository: NikkeRepository = mock()
    private val increaseNikkeAttractionCase = IncreaseNikkeAttractionCase(nikkeRepository)
    ...
    fun `should level up when nikke exist`() {
        ...
        val result = increaseNikkeAttractionCase.execute(id)

        assertThat(result.attraction.value).isEqualTo(2)
    ...
    fun `should throw a exception when nikke not exist`() {
        ...
        val exception = assertThrows<NikkeNotFoundException> {
            increaseNikkeAttractionCase.execute(id)
        }
