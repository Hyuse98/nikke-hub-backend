...
class LevelUpNikkeSkill2CaseTest {

    private val nikkeRepository: NikkeRepository = mock()
    private val levelUpNikkeSkill2Case = LevelUpNikkeSkill2Case(nikkeRepository)
    ...
    fun `should level up when nikke exist`() {
        ...
        val result = levelUpNikkeSkill2Case.execute(id)

        assertThat(result.skill2.value).isEqualTo(2)
    ...
    fun `should throw a exception when nikke not exist`() {
        ...
        val exception = assertThrows<NikkeNotFoundException> {
            levelUpNikkeSkill2Case.execute(id)
        }
